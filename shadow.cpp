...
static const int TEXTURE_TOP = 4;
static const int TEXTURE_BOTTOM = 6;

static int uniformWidth = -2;
static int uniformHeight = -2;
static int uniformBlur = -2;
...
Atlas *renderText(HBText text, unsigned int size, float x = 0, float y = 0) {
    ...
    for (unsigned int i = 0; i < glyphCount; ++i) {
        ...
        roww += bitmap.width + 1;
        rowh = std::max(rowh + TEXTURE_TOP + TEXTURE_BOTTOM, bitmap.rows);
    }
    tw = std::max(tw, roww);
    th += rowh;
    ...
    for (unsigned int i = 0; i < glyphCount; ++i) {
        ...
        float t0 = (oy - TEXTURE_TOP) / (float) th;
        ...
        float t1 = t0 + float(bitmap.rows + TEXTURE_BOTTOM) / atlas->h;
...
void initGL() {
    ...
    uniformWidth = glGetUniformLocation(program, "t_width");
    uniformHeight = glGetUniformLocation(program, "t_height");
    uniformBlur = glGetUniformLocation(program, "blur");
...
void drawText(Atlas *atlas) {
    glUniform1f(uniformWidth, atlas->w);
    glUniform1f(uniformHeight, atlas->h);
    glBindTexture(GL_TEXTURE_2D, atlas->texture1);
    glBindVertexArray(VAO);
